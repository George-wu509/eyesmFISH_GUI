function convertlsm2tiff(lsm)
% convert *.lsm file into *tiff images
% lsm: lsm file name
% tiff: 

if ismac||isunix==1
    lsmfolderfile=[pwd '/' lsm];
else
    lsmfolderfile=[pwd '\' lsm];
end
lsm_stack = tiffread(lsmfolderfile);
lsmfile=lsm;
...
if ismac||isunix==1
    tiffolder=strrep(lsmfile,'.lsm','/tif/');
elseif ispc==1
    tiffolder=strrep(lsmfile,'.lsm','\tif\');
end
